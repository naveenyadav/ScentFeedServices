...
@Component
public class DownVoteStep implements IAction {
    ...
    public DownVoteStep(PostRepository postRepository, LikeRepository likeRepository){
    ...
    public void perFormAction(EventData eventData){
        ...
        //Get like by UserID
        Mono<Long> dislikeMono = likeRepository.countLikesByUserId(likeUserId)
        ...
        dislikeMono.zipWith(postMono).log().subscribe(tuple -> onSuccess(tuple,  paramMap));
        //Save like
        //update post
    }
    ...
    public void onSuccess(Tuple2<Long, Post> tuple, Map<String, String> paramMap){
        String voteType = paramMap.get(VOTE_TYPE);
    ...
    public Mono<Post> processPost(Post post, String userId){
        ...
        if(result) {
            ...
            int upVoteHour = configServiceImpl.getPropertyValueAsInteger(GLOBAL_CONFIG, decrementExpiryDate(post));
    ...
    private String decrementExpiryDate(Post post){
        return PROP_POST_HOUR_DOWN_VOTE;
    }
}
